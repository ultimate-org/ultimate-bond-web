    ...
    try { 
        ...
        if (responseData.code === 200) {
           ...
           const data = {
            ...
            const loggingResponse = await fetch(`${ process.env.NEXT_PUBLIC_BASE_URL}login`, {
                method:"POST",
                headers: { 'Content-Type': 'application/json' },
            ...
            if (loggedUserData.code === 200) {
                ...
                const userInfoResponse = await fetch(`${ process.env.NEXT_PUBLIC_BASE_URL}parent/byUser/${userId}`,{method:"GET"});
...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${ process.env.NEXT_PUBLIC_BASE_URL}parent/child/${Number(id)}`);
        if (!response.ok) {
