...
const HEADLINE_GRADIENT =
  "linear-gradient(103.62deg, #ffb13d 0%, #ff7a18 26%, #ff4d8d 64%, #a24bff 100%)";
...
// Line breaks are hand-placed (not left to auto-wrap) so every card
// resolves to the same number of lines and the fixed card height below
// never has to clip or stretch to fit one card's text.
const HALO_CARDS: HaloCard[] = [
  {
    icon: HeartHandshake,
    content: (
      <>
        ...
        <br />
        <b className="font-semibold" style={{ color: "var(--site-heading)" }}>in tough situations?</b>
      </>
    ),
  },
  {
    ...
    content: (
      <>
        ...
        <br />
        decision making{" "}
        <b className="font-semibold" style={{ color: "var(--site-heading)" }}>
          in
          <br />
          relationships?
        </b>
...
export default function RealityCheck() {
  return (
    <section
      className="relative w-full overflow-hidden px-[clamp(1.25rem,4vw,3rem)] py-[clamp(3.5rem,7vw,6.5rem)] font-['Poppins',sans-serif] antialiased"
      style={{ background: "var(--site-page-bg)", color: "var(--site-text)" }}
    >
      <div className="relative z-[1] mx-auto max-w-[82.5rem]">
        {/* Pill */}
        <motion.div
          custom={0}
          initial="hidden"
        ...
        >
          <div
            className="inline-flex h-9 items-center gap-2.5 rounded-full border px-[18px]"
            style={{
          ...
          >
            <span
              className="h-[7.2px] w-[7.2px] flex-shrink-0 rounded-full bg-[#ff9a40]"
              style={{ boxShadow: "0 0 8.229px #ff9a40" }}
            />
            <span
              className="whitespace-nowrap font-['Outfit',sans-serif] text-[12.3px] font-semibold uppercase tracking-[1.97px]"
        ...
        <motion.p
          ...
          viewport={{ once: true, amount: 0.4 }}
          variants={fadeUp}
          className="mb-[clamp(18px,2vw,26px)] text-center text-[clamp(15px,1.05vw,19px)] leading-[1.5]"
          style={{ color: "var(--site-text-muted)" }}
        >
          Traditional education prepares children for{" "}
          <span
            className="relative whitespace-nowrap font-medium line-through decoration-[#5c5b68] decoration-2"
        ...
        <motion.h2
          ...
          viewport={{ once: true, amount: 0.4 }}
          variants={fadeUp}
          className="mx-auto mb-[clamp(28px,3.5vw,44px)] max-w-[60rem] text-center font-['Outfit',sans-serif] text-[clamp(32px,4.4vw,60px)] font-extrabold leading-[1.02] tracking-[-1.2px]"
        ...
        {/* Halo row */}
        <div className="grid grid-cols-1 gap-[clamp(20px,3vw,34px)] sm:grid-cols-2 lg:grid-cols-5 lg:gap-[clamp(14px,1.6vw,28px)]">
          {HALO_CARDS.map((card, i) => {
            ...
            return (
              <motion.div
                ...
                variants={fadeUp}
                className={
                  "relative flex items-end justify-center pt-[clamp(26px,3vw,42px)]" +
                  (i === 4 ? " sm:col-span-2 sm:mx-auto sm:w-1/2 lg:col-span-1 lg:w-full" : "")
                }
              >
                {/* dashed halo rings */}
                <span
                  className="pointer-events-none absolute left-1/2 top-1/2 aspect-square w-[96%] max-w-[251px] -translate-x-1/2 -translate-y-1/2 rounded-full"
                  style={{ border: "1px dashed var(--site-card-border)" }}
                />
                <span
                  className="pointer-events-none absolute left-1/2 top-1/2 aspect-square w-[118%] max-w-[309px] -translate-x-1/2 -translate-y-1/2 rounded-full"
                  style={{ border: "1px dashed var(--site-card-border)" }}
                />

                <article
                  className="group relative z-[1] flex h-[219px] w-full flex-col items-center overflow-hidden rounded-[26.9px] border px-[18px] pb-[24px] pt-[29px] text-center backdrop-blur-[23px] transition-all duration-500 ease-out hover:-translate-y-1.5"
                  style={{
                ...
                >
                  <span
                    className="mb-[22px] flex h-[63.6px] w-[63.6px] flex-shrink-0 items-center justify-center rounded-full border transition-transform duration-500 ease-out group-hover:scale-[1.06]"
                    style={{
                      borderColor: "var(--site-pill-border)",
                      background: "var(--site-pill-bg)",
                      boxShadow: "inset 0 1.029px 0 rgba(255,255,255,0.16)",
                  ...
                  >
                    <Icon size={28} strokeWidth={1.75} className="text-[#ff9a40]" />
                  </span>
                  <p
                    className="my-auto text-[17.1px] font-medium leading-[25.7px]"
                    style={{ color: "var(--site-text)" }}
                  >
                    {card.content}
                  </p>
