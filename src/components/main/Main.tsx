...
const STATS = [
  ...
  },
  {
    ...
    num: (
      <>
        9 <span className="font-medium text-[var(--hero-lead-color)]">in</span> 10
      </>
    ),
    accent: "violet",
    numColor: "#a24bff",
...
/** Masked gradient-stroke ring — the "sampled gradient stroke" trick from the
 *  reference design: a padding-box vs border-box mask difference leaves only
 *  the ring visible, so the border can be a multi-stop gradient instead of a
 *  flat color. `width` is a literal CSS length; `widthVar` is a CSS custom
 *  property name (takes priority over `width` when given). */
function GradientRing({
  ...
  return (
    <span
      ...
      style={{
        padding: widthVar ? `var(${widthVar})` : width,
...
export default function Hero() {
  ...
  return (
    <div className="relative flex min-h-[100svh] w-full flex-col overflow-hidden pt-[var(--nav-h,80px)] font-['Poppins',sans-serif] antialiased">
      <style>{`
        @keyframes orbit-travel { to { offset-distance: 100%; } }
        @keyframes card-float { 0%,100%{transform:translate3d(0,0,0)} 50%{transform:translate3d(0,-16px,0)} }
        @media (prefers-reduced-motion: reduce){
      ...
      {/* page background */}
      <div className="absolute inset-0 -z-10" style={{ background: "var(--hero-page-bg)" }} />
          ...
          <div className="relative z-[1] mx-auto grid w-full max-w-[1600px] grid-cols-1 items-center gap-8 px-5 sm:gap-10 sm:px-7 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] lg:gap-[clamp(20px,2.2vw,56px)] lg:px-[clamp(20px,3vw,72px)]">
            ...
            <div className="relative flex flex-col items-center text-center lg:items-start lg:text-left">
              ...
              <motion.div
                ...
                animate="visible"
                variants={fadeUp}
                className="relative z-[2] mb-[clamp(10px,1.4vh,20px)] inline-flex h-7 items-center gap-2 rounded-full bg-[var(--hero-pill-bg)] px-3 backdrop-blur-[var(--hero-glass-blur)] transition-colors duration-700 sm:h-8 sm:gap-2.5 sm:px-3.5 lg:h-9 lg:px-[18px]"
                style={{ boxShadow: "inset 0 1.244px 0 rgba(255,255,255,0.16)" }}
              >
                <GradientRing colorVar="--hero-pill-stroke" width="1.244px" />
                <span className="h-1.5 w-1.5 rounded-full bg-[#ff9a40] sm:h-2 sm:w-2" />
                <span className="font-['Outfit',sans-serif] text-[10px] font-semibold uppercase tracking-[1.4px] text-[var(--hero-pill-text)] sm:text-[11px] sm:tracking-[1.6px] lg:text-[12px] lg:tracking-[2px]">
                  Family Growth Platform
                </span>
              </motion.div>
              ...
              <motion.h1
                ...
                animate="visible"
                variants={fadeUp}
                className="relative z-[2] mb-[clamp(10px,1.6vh,20px)] font-['Outfit',sans-serif] text-[clamp(26px,7vw,38px)] font-extrabold leading-[1.05] tracking-[-0.6px] sm:text-[clamp(34px,5vw,52px)] lg:text-[clamp(40px,4.2vw,64px)] lg:tracking-[-1.4px]"
              >
                <span
                  className="bg-clip-text text-transparent"
                  style={{ backgroundImage: "var(--hero-h-main-bg)" }}
                >
                  Ordinary Moments.
                </span>
                <br />
                <span
                  className="bg-clip-text text-transparent"
                  style={{ backgroundImage: "var(--hero-h-accent-bg)" }}
                >
                  Extraordinary
                </span>{" "}
                <span
              ...
              >
                ...
                time. Spend just 10 mins a day with simple, age appropriate
                activities designed to build your child&apos;s{" "}
                <span className="text-[var(--hero-lead-accent)]">
                  emotional strength, resilience, and thinking skills,
                </span>{" "}
                while bridging the gap between Knowing and Doing.
              </motion.p>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    boxShadow: "inset 0 1.244px 0 rgba(255,255,255,0.48)",
                  }}
                >
                  Get the App
                </button>
                ...
                <button
                  ...
                  className="group inline-flex h-[3.25rem] sm:h-[3.5rem] cursor-pointer items-center gap-2.5 border-none bg-transparent px-4 sm:px-5 font-['Outfit',sans-serif] text-[19px] sm:text-[19px] font-bold tracking-[0.2px] text-[var(--hero-ghost-color)] transition-colors duration-300 ease-in-out hover:text-[var(--hero-ghost-hover)] hover:opacity-100"
                >
              ...
              <motion.div
                ...
                animate="visible"
                variants={fadeUp}
                className="relative z-[1] grid w-full max-w-full grid-cols-2 gap-3 text-left sm:gap-5 lg:max-w-[600px] lg:gap-5 xl:max-w-[760px] xl:grid-cols-4 xl:gap-[26px]"
              >
                {STATS.map((s) => (
                  <div
                    key={s.label}
                  ...
                  >
                    ...
                    {/* bottom radial glow — per-card accent color, same both themes */}
                    <span
                      aria-hidden="true"
                      className="pointer-events-none absolute -inset-x-[1%] -bottom-[75%] z-0 h-[87%] opacity-[0.55]"
                      style={{
                        background: `radial-gradient(ellipse 50% 100% at 50% 100%, var(--hero-glow-${s.accent}) 0%, transparent 72%)`,
                      }}
                    />
                    <GlassEdgeTop />
                    <GlassEdgeLeft />
                    <div
                      className="inline-flex h-9 w-9 flex-shrink-0 items-center justify-center self-start rounded-[18px] text-[15px] text-white backdrop-blur-[10px] transition-transform duration-1100 ease-smooth group-hover:scale-110 sm:h-11 sm:w-11 sm:rounded-[20px] sm:text-[17px] lg:h-12 lg:w-12 lg:rounded-[22px] lg:text-[19px]"
                      style={{
                        background: `var(--hero-badge-${s.accent})`,
                        border: `1px solid ${s.badgeStroke}`,
                        boxShadow: `inset 0 1px 0 ${s.badgeInset}, inset 0 -1px 0 rgba(255,255,255,0.08), 0 4px 14px rgba(0,0,0,0.20)`,
                      }}
                    >
                    ...
                    <div className="mt-auto flex flex-col gap-1.5">
                      <div
                        className="text-[clamp(19px,4.8vw,24px)] font-semibold leading-none sm:text-[26px]"
                        style={{ color: `var(--hero-stat-num, ${s.numColor})` }}
                      >
                        {s.num}
                      </div>
                      <div className="text-[clamp(12px,2.9vw,14px)] font-normal leading-[1.35] text-[var(--hero-stat-label)] sm:text-[15px] sm:leading-[1.4]">
                        {s.label}
                      </div>
                    </div>
                  </div>
            ...
            <motion.div
              ...
              animate="visible"
              variants={fadeUp}
              className="relative z-[2] hidden h-full w-full lg:block lg:min-h-[clamp(24rem,58vh,38rem)]"
            >
              {TESTIMONIALS.map((t) => (
                <div
                  key={t.id}
                  className={`group absolute z-[3] flex w-[clamp(19rem,23.44vw,28.125rem)] flex-col gap-2 rounded-[20px] bg-[var(--hero-float-bg)] p-[16px_18px_18px] backdrop-blur-[var(--hero-glass-blur)] transition-colors duration-700 hover:[animation-play-state:paused] ${t.posClass}`}
                  style={{
                ...
                >
                  <GradientRing colorVar="--hero-float-stroke" widthVar="--hero-float-border-w" />
                  <GlassEdgeTop />
                  <div className="flex items-center gap-3">
                    <span
                    ...
                    <div className="text-[13px] leading-none tracking-[1.6px] text-[var(--hero-star-color)]">
                      ★★★★★
                    </div>
                  </div>
                  <p className="m-0 text-[14px] font-medium leading-[1.45] text-[var(--hero-quote-color)]">
                    &ldquo;{t.quote}&rdquo;
                  </p>
                  <p className="m-0 text-[12.5px] font-normal text-[var(--hero-author-color)]">
                    {t.author}
                  </p>
                  <span
                    className="absolute -right-3 -top-3.5 z-[4] inline-flex h-[34px] w-[34px] items-center justify-center rounded-[17px] text-[15px] text-white backdrop-blur-[10px] transition-transform duration-1000 ease-smooth group-hover:scale-105"
