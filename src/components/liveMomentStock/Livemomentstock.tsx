...
//  *
//  * CLIENT COMPONENT — unavoidable: the entire component IS the interactivity
//  * (setInterval arrivals, setTimeout auto-dismiss, rAF enter animation, click
//  * to dismiss). A server component renders once and never updates, so there
//  * would be no stream at all.
//  *
//  * FIXED: cards no longer arrive in a fast burst on mount. Previously four
//  * seed timers fired 1.2s apart and then the interval dropped to 14s — a
//  * jarring cadence change. Now a single card seeds the stack, and every
//  * subsequent card arrives on the same steady `streamIntervalMs` rhythm.
//  */
...
//     // Seed a single card shortly after mount, then hand off to the steady
//     // stream. Previously four seed timers fired ~1.2s apart, producing a
//     // rapid burst before the interval slowed to streamIntervalMs. Now every
//     // card — including the first — arrives on the same even cadence.
...
export default function LiveMomentsStack({
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[290px] h-[160px] z-[9999] pointer-events-none font-sans max-sm:bottom-5 max-sm:right-4 max-sm:left-4 max-sm:w-auto">
      {cards.map((card) => {
        const theme = themeStyles[card.theme] || themeStyles.readometer;
        const posClass = card.exiting
          ? EXITING_STYLE
        ...
        return (
          <div
            key={card.id}
            className={`group absolute bottom-0 right-0 w-[280px] px-[14px] py-3 rounded-[18px] cursor-pointer pointer-events-auto border-[0.5px] origin-bottom-right will-change-transform backdrop-blur-[40px] backdrop-saturate-[180%] max-sm:w-full hover:!-translate-y-1 hover:!scale-[1.02] ${theme.card} ${exitTransition} ${posClass}`}
          >
            {/* Inner highlight overlay */}
            <div className="pointer-events-none absolute inset-0 rounded-[18px] bg-[linear-gradient(135deg,rgba(255,255,255,0.15)_0%,rgba(255,255,255,0.05)_40%,transparent_60%,rgba(255,255,255,0.03)_100%)]" />
            ...
            <div className="relative z-[1] flex items-center gap-2.5">
              {/* Icon bubble */}
              <div
                className={`w-9 h-9 rounded-[12px] flex items-center justify-center shrink-0 text-[18px] backdrop-blur-[10px] border-[0.5px] shadow-[inset_0_1px_0_rgba(255,255,255,0.2)] ${theme.icon}`}
              >
                {card.icon}
              </div>
              ...
              {/* Text body */}
              <div className="flex-1 min-w-0">
                <div className="text-[11.5px] leading-[1.35] text-white/95 font-normal">
                  <strong className="font-semibold text-white">{card.name}</strong>
                  {card.city ? ` from ${card.city} ` : ' '}
                  {card.detail}
                </div>
                <div
                  className={`text-[9px] mt-0.5 tracking-[0.6px] uppercase font-semibold ${theme.meta}`}
                >
                  {card.meta} · just now
                </div>
              </div>
